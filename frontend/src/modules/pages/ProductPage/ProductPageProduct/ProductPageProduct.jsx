  ...
  return (
    <div className={styles.product_container}>
          ...
          <div className={styles.product_title_date}>
            <div className={styles.product_title}>
              <h2 className={styles.product_title_name}>
                <Link to={`/products/${productPageProduct.id}`}>
                  {productPageProduct.name}
                </Link>
              </h2>
              <h2 className={styles.product_title_price}>
                {productPageProduct.price} &#8381;
              </h2>
            </div>
            <p className={styles.product_description}>
              {productPageProduct.description}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.product_bottom}>
          ...
          {productPageProduct.review_count != 0 ? (
            <span>
              <img
                ...
                alt="Star icon"
              />{" "}
              {productPageProduct.rating} | {productPageProduct.review_count}{" "}
              {pluralizedReviews}
            </span>
          ) : (
        ...
        <div>
          {!productPageProduct.is_favorite ? (
            <img
              className={styles.product_icon}
              src={favorites_0_icon}
              alt="Favorites"
              onClick={() =>
                addFavorite(productPageProduct.id, setProductPageProduct)
              }
            />
          ) : (
            <img
              ...
              onClick={() =>
                deleteFavorite(
                  productPageProduct.favorite_id,
          ...
          {!productPageProduct.is_cart_product ? (
            <button
              ...
              onClick={() =>
                addCartProduct(productPageProduct.id, setProductPageProduct)
              }
            >
              Купить
            </button>
