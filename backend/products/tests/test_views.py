...
class TestFavoriteProductViewSet:
    ...
    def test_list_favorites_authenticated(self, api_auth_client, favorite_product11, url_favorite_product_list):
        response = api_auth_client.get(url_favorite_product_list)
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]["product"]["id"] == favorite_product11.product.id

    #create
    def test_create_favorite_product_unauthenticated(self, api_client, product1, url_favorite_product_list):
        response = api_client.post(url_favorite_product_list, {"product": product1.id})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_create_favorite_product_nonexistent_product(self, api_auth_client, url_favorite_product_list):
        response = api_auth_client.post(url_favorite_product_list, {"product": 999})
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    #retrieve
    def test_retrieve_favorite_product_unauthorized(self, api_client, favorite_product11, url_favorite_product_detail):
        response = api_client.get(url_favorite_product_detail(favorite_product11.id))
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
        
    def test_retrieve_favorite_product_authenticated(self, api_auth_client, favorite_product11, url_favorite_product_detail):
        response = api_auth_client.get(url_favorite_product_detail(favorite_product11.id))
        assert response.status_code == status.HTTP_200_OK
        assert response.data["product"]["id"] == favorite_product11.product.id

    def test_retrieve_favorite_product_different_user(self, api_auth_client, favorite_product21, url_favorite_product_detail):
        response = api_auth_client.get(url_favorite_product_detail(favorite_product21.id))
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_retrieve_nonexistent_favorite_product(self, api_auth_client, url_favorite_product_detail):
        response = api_auth_client.get(url_favorite_product_detail(999))
        assert response.status_code == status.HTTP_404_NOT_FOUND

    #update
    def test_update_favorite_product_not_allowed(self, api_auth_client, favorite_product11, product2, url_favorite_product_detail):
        response = api_auth_client.put(url_favorite_product_detail(favorite_product11.id), {"product": product2.id})
        assert response.status_code == status.HTTP_405_METHOD_NOT_ALLOWED
...
class TestCartProductViewSet:
    #list
    def test_list_cart_products_unauthenticated(self, api_client, url_cart_product_list):
        response = api_client.get(url_cart_product_list) 
        
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_list_cart_products_authenticated(self, api_auth_client, url_cart_product_list, cart_product11):
        response = api_auth_client.get(url_cart_product_list)
        
        assert response.status_code == status.HTTP_200_OK
        assert "cart_products" in response.data
        assert "total_quantity" in response.data
        assert "total_price" in response.data
        assert len(response.data["cart_products"]) == 1
    
    def test_list_cart_products_empty(self, api_auth_client, url_cart_product_list):
        response = api_auth_client.get(url_cart_product_list)
        
        assert response.status_code == status.HTTP_200_OK
        assert response.data["cart_products"] == []
        assert response.data["total_quantity"] == 0
        assert response.data["total_price"] == 0
    
    #create
    def test_create_cart_product(self, api_auth_client, url_cart_product_list, user1, product1):    
        response = api_auth_client.post(url_cart_product_list, {"product": product1.id})
        
        assert response.status_code == status.HTTP_201_CREATED
        assert CartProduct.objects.filter(user=user1, product=product1).exists()
        assert response.data["quantity"] == 1
    
    def test_create_cart_product_without_quantity(self, api_auth_client, url_cart_product_list, user1, product1):
        response = api_auth_client.post(url_cart_product_list, {"product": product1.id})

        assert response.status_code == status.HTTP_201_CREATED
        cart_product = CartProduct.objects.get(user=user1, product=product1)
        assert cart_product.quantity == 1
    
    def test_create_cart_product_unique_together(self, api_auth_client, url_cart_product_list, cart_product11):
        response = api_auth_client.post(url_cart_product_list, {"product": cart_product11.product.id})
        
        assert response.status_code == status.HTTP_400_BAD_REQUEST
    
    #update
    def test_update_cart_product_quantity(self, api_auth_client, url_cart_product_detail, cart_product11):
        response = api_auth_client.patch(url_cart_product_detail(cart_product11.id), {"quantity": 5})

        assert response.status_code == status.HTTP_200_OK
        cart_product11.refresh_from_db()
        assert cart_product11.quantity == 5
    
    def test_update_cart_product_another_user(self, api_auth_client, url_cart_product_detail, user2, product1):
        cart_product21 = CartProduct(user=user2, product=product1)
        response = api_auth_client.patch(url_cart_product_detail(cart_product21.id), {"quantity": 5})

        assert response.status_code == status.HTTP_404_NOT_FOUND
    
    #delete
    def test_delete_cart_product(self, api_auth_client, url_cart_product_detail, cart_product11):   
        response = api_auth_client.delete(url_cart_product_detail(cart_product11.id))
        
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not CartProduct.objects.filter(id=cart_product11.id).exists()
    
    def test_delete_cart_product_another_user(self, api_auth_client, url_cart_product_detail, cart_product21):
        response = api_auth_client.delete(url_cart_product_detail(cart_product21.id))
        
        assert response.status_code == status.HTTP_404_NOT_FOUND
        assert CartProduct.objects.filter(id=cart_product21.id).exists()
    
    #delete_multiply
    def test_delete_multiple_cart_products(self, api_auth_client, url_cart_product_delete_multiple, user1, cart_product11, cart_product12):
        response = api_auth_client.delete(url_cart_product_delete_multiple(cart_product11.id, cart_product12.id))
        
        assert response.status_code == status.HTTP_204_NO_CONTENT
        assert not CartProduct.objects.filter(user=user1).exists()
    
    def test_delete_multiple_empty_list(self, api_auth_client, url_cart_product_delete_multiple, user1, cart_product11):
        response = api_auth_client.delete(url_cart_product_delete_multiple())
        
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert CartProduct.objects.filter(user=user1).count() == 1
    
    def test_delete_multiple_with_invalid_ids(self, api_auth_client, url_cart_product_delete_multiple, user1, cart_product11):
        response = api_auth_client.delete(url_cart_product_delete_multiple(999, 1000))
